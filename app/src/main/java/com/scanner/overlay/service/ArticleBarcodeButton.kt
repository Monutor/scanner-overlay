...
class ArticleBarcodeButton(
    ...
    private var articleButtonSizePx: Int
    ...
    init {
        ...
        articleButtonSizePx = (60 * displayMetrics.density).toInt()
        ...
        button = ImageView(context).apply {
            setImageResource(R.drawable.ic_article_barcode)
    ...
    private fun resolveInitialX(): Int {
        ...
        val shelfX = prefs.getInt(PREF_SHELF_X, -1)
        val articleX = prefs.getInt(PREF_ARTICLE_X, -1)
        ...
        val fallbackShelfX = (mainLeftEdge - shelfButtonSizePx - marginPx).coerceAtLeast(marginPx)
        val shelfLeftEdge = if (shelfX >= 0) shelfX else fallbackShelfX
        val fallbackArticleX = (shelfLeftEdge - articleButtonSizePx - marginPx).coerceAtLeast(marginPx)
        val articleLeftEdge = if (articleX >= 0) articleX else fallbackArticleX
        return (articleLeftEdge - buttonSizePx - marginPx).coerceAtLeast(marginPx)
    }
    ...
    private fun launchActivity() {
        val intent = Intent(context, ArticleBarcodeActivity::class.java).apply {
    ...
    companion object {
        private const val PREF_SIZE = "article_barcode_size_dp"
        private const val DEFAULT_SIZE_DP = 60
        private const val PREF_X = "article_barcode_x"
        private const val PREF_Y = "article_barcode_y"
        private const val PREF_MAIN_X = "floating_button_x"
        private const val PREF_MAIN_Y = "floating_button_y"
        private const val PREF_SHELF_X = "shelf_button_x"
        private const val PREF_ARTICLE_X = "article_button_x"
    }
}
